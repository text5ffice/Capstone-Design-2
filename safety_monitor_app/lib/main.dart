...
void _handleMessage(RemoteMessage message) {
  if (message.data.isNotEmpty) {
    final alert = AlertModel(
      ...
      imageUrl: message.data['imageUrl'] ?? 'http://192.168.1.108:5000/static/images/test.jpg',
    );
...
void _sendTokenToServer(String token) async {
  try {
    final response = await http.post(
      Uri.parse('http://192.168.1.108:5000/api/token'),
...
class _MainDashboardState extends State<MainDashboard> {
  Future<List<AlertModel>> fetchAlerts() async {
    final response = await http.get(Uri.parse('http://192.168.1.108:5000/api/alerts'));
    if (response.statusCode == 200) {
...
class _WorkerListScreenState extends State<WorkerListScreen> {
  Future<List<WorkerModel>> fetchWorkers() async {
    final response = await http.get(Uri.parse('http://192.168.1.108:5000/api/workers'));
    if (response.statusCode == 200) {
      List<dynamic> jsonResponse = jsonDecode(utf8.decode(response.bodyBytes));
